      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <div className="grid gap-4 py-4">
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <Input id="name" value={editedLevel.nome} onChange={e => handleChange("nome", e.target.value)} className="col-span-3" />
          </div>
          <div className="grid grid-cols-4 items-center gap-4">
            <Label htmlFor="min" className="text-right">
              Mínimo
            </Label>
            <Input id="min" type="number" min="0" value={editedLevel.minimo} onChange={e => handleChange("minimo", parseInt(e.target.value, 10) || 0)} className="col-span-3" />
          </div>
          <div className="grid grid-cols-4 items-center gap-4">
            <Label htmlFor="max" className="text-right">
              Máximo
            </Label>
            <Input id="max" type="number" min="0" value={editedLevel.maximo} onChange={e => handleChange("maximo", parseInt(e.target.value, 10) || 1)} className="col-span-3" />
          </div>
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <div className="col-span-3 flex items-center">
              <Switch id="active" checked={editedLevel.ativo} onCheckedChange={checked => handleChange("ativo", checked)} />
            </div>
          </div>
        </div>
        <DialogFooter>
...
export function MenuLevelList({
  ...
  return <>
      ...
      {editingLevel && <LevelEditForm level={editingLevel} open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen} onSave={handleSaveEdit} />}
    </>;
}